...
export class FeeStructureService extends BaseService<FeeStructure> {
    ...
    try {
      ...
      if (!this.isValidObjectId(createDto.classId)) {
        throw new BadRequestException(
          `Invalid classId format: ${createDto.classId}`,
        );
      }
      ...
      for (const component of createDto.feeComponents) {
        if (!this.isValidObjectId(component.feeCategory)) {
          throw new BadRequestException(
            `Invalid feeCategory format: ${component.feeCategory}`,
          );
        }
      }
      ...
      const validationData: ValidateFeeStructureInput = {
        ...
        feeComponents: createDto.feeComponents.map((component) => ({
          ...component,
          feeCategory: Types.ObjectId.createFromHexString(
  ...
  async getFeeStructures(
    ...
    if (query.academicYear) {
      filter.academicYear = { $regex: query.academicYear, $options: 'i' };
    }
  ...
  async updateFeeStructure(
    ...
    const updatedStructure = await repository.findByIdAndUpdate(id, {
      ...
      classId: updateDto.classId
        ...
        : existing.classId,
      feeComponents:
        updateDto.feeComponents?.map((component) => ({
          ...component,
          feeCategory: Types.ObjectId.createFromHexString(
            component.feeCategory,
          ),
    ...
    const newComponents = sourceStructure.feeComponents.map((component) => {
      ...
      if (override) {
        return {
          ...
          lateChargeType:
            override.newLateChargeType || component.lateChargeType,
          lateChargeValue:
            override.newLateChargeValue || component.lateChargeValue,
        };
      }
  ...
  private async validateClassIds(
    ...
    const classes = await classRepo.find({
      _id: {
        $in: classIds.map((id) => Types.ObjectId.createFromHexString(id)),
      },
      isActive: true,
    });
    ...
    if (classes.length !== classIds.length) {
      throw new BadRequestException(
        'One or more class IDs are invalid or inactive',
      );
    }
  }
